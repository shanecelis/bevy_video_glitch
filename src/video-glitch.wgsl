...
fn fragment(
    ...
    let texColor: vec4<f32> = textureSample(texture, our_sampler, vec2<f32>(xpos, uv.y));
    ...
    fragColor.g = mix(fragColor.r, textureSample(texture, our_sampler, vec2<f32>(xpos + noise * 0.05, uv.y)).g, 0.25);
    // fragColor.b = mix(fragColor.r, textureSample(iChannel0, vec2<f32>(xpos - noise * 0.05, uv.y)).b, 0.25);
    fragColor.b = mix(fragColor.r, textureSample(texture, our_sampler, vec2<f32>(xpos - noise * 0.05, uv.y)).b, 0.25);
